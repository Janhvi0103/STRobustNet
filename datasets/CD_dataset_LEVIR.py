...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        self.list_path = os.path.join(self.root_dir, self.split, LIST_FOLDER_NAME, self.split+'.txt')
        ...
        if is_train:
            self.augm = CDDataAugmentation(
                ...
                mean=[[0.45026174, 0.44666728, 0.38134828],[0.34552237, 0.33819625, 0.2888154]],
                std=[[0.21711645, 0.20401348, 0.18666],[0.15759413, 0.15198629, 0.14440957]]
            )
        else:
            self.augm = CDDataAugmentation(
                img_size=self.img_size,
...
class CDDataset_LEVIR(ImageDataset):

    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(CDDataset_LEVIR, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                        to_tensor=to_tensor)
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'test':
            ...
            label = np.array(Image.open(L_path).convert('L'), dtype=np.uint8) # 为0-1
        else:
            L_path = get_label_path(self.root_dir, self.split, self.img_name_list[index % self.A_size])
