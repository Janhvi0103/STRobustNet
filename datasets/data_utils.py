...
class CDDataAugmentation:
    ...
    def transform(self, imgs, labels, to_tensor=True):
        ...
        if self.img_size is None:
            self.img_size = None
        if self.random_exchange and random.random()>0.5:
            imgs.reverse()
            # labels.reverse()
        ...
        if self.with_scale_random_crop:
           ...
           # print(box)
            imgs = [pil_crop(img, box, cropsize=self.img_size, default_value=0)
                    for img in imgs]
            labels = [pil_crop(img, tuple(i  for i in box), cropsize=self.img_size, default_value=255)
                    for img in labels]
        ...
        if to_tensor:
            ...
            imgs = [TF.normalize(imgs[0], mean=self.mean[0],std=self.std[0]),
                    TF.normalize(imgs[1], mean=self.mean[1],std=self.std[1])]
                    # for img in imgs]
