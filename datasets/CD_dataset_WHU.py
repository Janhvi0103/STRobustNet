...
"""
# WHU
IMG_FOLDER_NAME = "before"
IMG_POST_FOLDER_NAME = 'after'
LIST_FOLDER_NAME = 'list'
ANNOT_FOLDER_NAME = "change_label"
...
def get_img_post_path(root_dir,img_name):
    return os.path.join(root_dir,IMG_POST_FOLDER_NAME, "after."+img_name)


def get_img_path(root_dir, img_name):
    return os.path.join(root_dir, IMG_FOLDER_NAME, "before."+img_name)


def get_label_path(root_dir, img_name):
    return os.path.join(root_dir, ANNOT_FOLDER_NAME, "change_label."+img_name)
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        if is_train:
            self.augm = CDDataAugmentation(
                ...
                mean=[[0.48533878, 0.44473675, 0.3873848],[0.48430178, 0.48372635, 0.4614529]],
                std=[[0.17349541, 0.16978146, 0.17308392],[0.21380359, 0.20338096, 0.21925654]]
            )
        else:
            self.augm = CDDataAugmentation(
                img_size=self.img_size,
...
class CDDataset_WHU(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        label = np.array(Image.open(L_path).convert('L'), dtype=np.uint8)  # 对于WHU数据集，label为0-255
        #  二分类中，前景标注为255
